...
// ──────────────────────────────────────
// MARKDOWN EXPORT  (for Obsidian / Notion / any notes app)
// ──────────────────────────────────────
// Pure builder — no DOM/localStorage access, so it can be unit-tested.
// `notesText` and `bookmarks` are passed in by the caller.
function _buildStudyGuideMarkdown(d, notesText, bookmarks) {
    d = d || {};
    notesText = notesText || '';
    bookmarks = bookmarks || [];
    const letters = ['A', 'B', 'C', 'D'];
    const date = new Date().toISOString().slice(0, 10);
    const videoUrl = d.video_id ? `https://youtube.com/watch?v=${d.video_id}` : '';
    // fmtSecs is global in the app; fall back to a local formatter under test.
    const fmt = (typeof fmtSecs === 'function') ? fmtSecs : function (s) {
    ...
    // ── YAML front-matter (Obsidian/Notion friendly) ──
    lines.push('---');
    lines.push(`title: "${(d.title || 'Study Guide').replace(/"/g, "'")}"`);
    if (d.author) lines.push(`author: "${d.author.replace(/"/g, "'")}"`);
    if (videoUrl) lines.push(`source: ${videoUrl}`);
    if (d.difficulty) lines.push(`difficulty: ${d.difficulty}`);
    lines.push(`exported: ${date}`);
    lines.push('tags: [lecturedigest, study-guide]');
    lines.push('---');
    lines.push('');

    lines.push(`# ${d.title || 'Study Guide'}`);
    lines.push('');
    if (videoUrl) lines.push(`> 🔗 [Watch on YouTube](${videoUrl})`);
    lines.push('');
    ...
    // ── Key Takeaways ──
    if ((d.key_takeaways || []).length) {
        lines.push('## ✅ Key Takeaways');
        lines.push('');
        d.key_takeaways.forEach(t => lines.push(`- ${t}`));
        lines.push('');
    }

    // ── Chapter Timeline ──
    if ((d.topics || []).length) {
        lines.push('## 🗺️ Chapter Timeline');
        lines.push('');
        d.topics.forEach(t => {
            const ts = t.timestamp_str ? `\`${t.timestamp_str}\` ` : '';
            lines.push(`### ${ts}${t.emoji || '📌'} ${t.title}`);
            if (t.summary) { lines.push(''); lines.push(t.summary); }
            lines.push('');
        });
    }
    ...
    if ((d.highlights || []).length) {
        ...
        d.highlights.forEach(h => {
            const ts = h.timestamp_str ? `\`${h.timestamp_str}\` ` : '';
            lines.push(`- **${ts}${h.title}**${h.description ? ' — ' + h.description : ''}`);
        });
        lines.push('');
    }
    ...
    if (bookmarks.length) {
        ...
        bookmarks.forEach(bm => {
            const ts = typeof bm.time === 'number' ? fmt(bm.time) : (bm.timestamp_str || '');
            const lbl = bm.label || bm.note || bm.title || '';
            lines.push(`- \`${ts}\` ${lbl}`);
        });
        lines.push('');
    }
    ...
    lines.push('---');
    lines.push('*Generated by LectureDigest · Powered by Gemini AI*');
...
function exportMarkdown() {
    ...
    let notesText = '';
    try { notesText = localStorage.getItem('lectureDigest_note_' + d.video_id) || ''; } catch (e) {}

    let bookmarks = [];
    try { bookmarks = JSON.parse(localStorage.getItem('lectureDigest_bookmarks_' + d.video_id) || '[]'); } catch (e) {}

    const content = _buildStudyGuideMarkdown(d, notesText, bookmarks);
    const filename = (typeof slugify === 'function' ? slugify(d.title) : 'study-guide') + '.md';

    if (typeof downloadFile === 'function') {
        downloadFile(content, filename, 'text/markdown;charset=utf-8');
    } else {
        // Fallback if flashcard.js helpers aren't loaded
    ...
    if (typeof showToast === 'function') showToast('⬇ Đã xuất Markdown', 2000);
}
