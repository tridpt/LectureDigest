...
if (typeof SECTION_IDS !== 'undefined' && !SECTION_IDS.includes('analyticsSection')) {
    SECTION_IDS.push('analyticsSection');
}
if (typeof SPA_ROUTES !== 'undefined') SPA_ROUTES['analyticsSection'] = '/analytics';

var _analyticsPrevSection = 'hero';

function openAnalytics() {
    _analyticsPrevSection = (typeof SECTION_IDS !== 'undefined' ? SECTION_IDS : []).find(function(id) {
        var el = document.getElementById(id);
        return el && !el.classList.contains('hidden');
    }) || 'hero';
    if (typeof showSection === 'function') showSection('analyticsSection');
...
function closeAnalytics() {
    var el = document.getElementById('analyticsSection');
    if (el) el.classList.add('hidden');
    if (typeof showSection === 'function') showSection(_analyticsPrevSection);
    window.scrollTo({ top: 0, behavior: 'instant' });
}
...
function renderAnalytics() {
    var g = {};
    try { g = loadGamif(); } catch(e) {}
    var history = [];
    try { history = JSON.parse(localStorage.getItem('lectureDigest_history') || '[]'); } catch(e) {}

    var renders = [
        ['anStatCards',   function() { _anRenderStatCards(g, history); }],
        ['anHeatmap',     function() { _anRenderHeatmap(g); }],
        ['anQuizTrend',   function() { _anRenderQuizTrend(history); }],
        ['anWeeklyReport',function() { _anRenderWeeklyReport(g, history); }],
        ['anTopicMastery',function() { _anRenderTopicMastery(history); }],
        ['anStudyHours',  function() { _anRenderStudyHours(g); }],
    ];
    renders.forEach(function(r) {
        try { r[1](); } catch(e) { console.error('[Analytics] ' + r[0] + ':', e); }
    });
}
...
function _anRenderStatCards(g, history) {
    var el = document.getElementById('anStatCards');
    if (!el) return;
    ...
    history.forEach(function(h) {
        ...
        if (typeof loadProgress === 'function') {
            ...
            (prog.quizSessions || []).forEach(function(qs) { if (qs.pct != null) scores.push(qs.pct); });
        }
    });
    ...
    history.forEach(function(h) {
        ...
        if (d >= weekStart) weekVideos++;
    });

    // Study days this month
    var monthDays = 0;
    var monthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0,10);
    (g.studyDates || []).forEach(function(d) { if (d >= monthStart) monthDays++; });

    var cards = [
        { icon: '📚', val: g.totalVideos || 0, label: 'Tổng video', sub: weekVideos + ' tuần này', color: '#8b5cf6' },
        { icon: '🧠', val: avgScore + '%', label: 'Điểm quiz TB', sub: scores.length + ' lần làm', color: '#10b981' },
        { icon: '🔥', val: g.currentStreak || 0, label: 'Streak', sub: 'Kỷ lục: ' + (g.longestStreak || 0), color: '#f59e0b' },
        { icon: '📅', val: monthDays, label: 'Ngày học tháng này', sub: (g.totalStudyDays || 0) + ' tổng', color: '#60a5fa' },
        { icon: '🍅', val: g.pomoSessions || 0, label: 'Pomodoro', sub: (g.pomoTotalMin || 0) + ' phút', color: '#ef4444' },
    ];

    el.innerHTML = cards.map(function(c) {
        return '<div class="an-stat" style="--an-accent:' + c.color + '">' +
            '<div class="an-stat-icon">' + c.icon + '</div>' +
            '<div class="an-stat-val">' + c.val + '</div>' +
            '<div class="an-stat-label">' + c.label + '</div>' +
            '<div class="an-stat-sub">' + c.sub + '</div>' +
        '</div>';
    }).join('');
}
